...
function validateResult(raw: unknown): EvaluationResult | null {
...
export const aiService = {
  async evaluate(params: EvalParams): Promise<EvaluationResult> {
    ...
    const cachedModel = await fetchCachedModelAnswer(params.questionId, params.level);
    const userPrompt = [
    ...
    for await (const chunk of stream) {
      ...
      if (delta) {
        full += delta;
        await params.hub.publish(channel, { type: "token", delta });
      }
    }
    ...
    await cacheModelAnswer(params.questionId, params.level, result.modelAnswer);
    return result;
  },
};

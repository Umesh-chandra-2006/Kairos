 ...
 * summaries, then drain the notification outbox. Ticks every minute
 * (reminders are matched to the exact HH:MM).
 */
export async function runScheduledJobs(): Promise<{ reminders: number; summaries: number; sent: number }> {
  const db = getDb();
  ...
  return { reminders, summaries, sent };
}
...
export function registerScheduler(intervalMs = 60_000): NodeJS.Timeout {
  ...
  timer = setInterval(() => {
    runScheduledJobs()
      .then(({ reminders, summaries, sent }) => {
        if (reminders > 0 || summaries > 0 || sent > 0) {
          logger.info({ reminders, summaries, sent }, "scheduled jobs completed");
        }
      })
